...
def create_video_mb_source(map_files, num_channels, image_height, image_width, num_classes):
	...
	for i, map_file in enumerate(map_files):
		# Check if source i correspond to map file i
		assert(i+1==int(map_file.split('Map_')[1].split('.')[0]))
...
def train_model(train_mapFiles, output_dir, log_file, model_name, profiling=True):
	...
	p.start()
	train_reader = create_video_mb_source(train_mapFiles, 1, image_height, image_width, num_classes)
	p.terminate()
