...
class TrainReader(object):
	...
	def __init__(self, mapFiles, maxPartitions, network, sample_count, epoch_size):
		...
		self.trainModule   = int(sample_count/(epoch_size/maxPartitions))+1
...
def train_model(train_mapFiles, output_dir, log_file, model_name, profiling=True):
	...
	print('Total number of samples seen: {}\n'.format(sample_count))
	...
	for epoch in range(max_epochs):	 # loop over epochs
		while sample_count < epoch_size:		 # loop over minibatches in the epoch
			...
			if((trainer.previous_minibatch_sample_count > 1000) and reader.hasNext()):
...
if __name__ == '__main__':
	...
	# For training
	newModelName   = "VVG16_videoOF-test"
