...
def create_video_mb_source(map_files, num_channels, image_height, image_width, num_classes):
	...
	print(map_files)
	with open(map_files[0], 'r') as file:
		epoch_size = (len(file.readlines())-1)
	print(epoch_size)
	...
	return MinibatchSource(sources, max_sweeps=1, randomize=False), epoch_size
...
def train_model(train_mapFiles, output_dir, log_file, model_name, profiling=True):
	...
	p.start()
	train_reader, epoch_size = create_video_mb_source(train_mapFiles, 1, image_height, 
														image_width, num_classes)
	p.terminate()
	...
	training_session(
		...
		checkpoint_config=CheckpointConfig(filename=os.path.join(output_dir, 'Models', model_name), restore=True),
	).train()
...
if __name__ == '__main__':
	...
	if args.logdir is not None:
		logFile = args.logdir
	map_dir = os.path.join(data_dir, "OF_mapFiles_big")
	...
	# For evaluation
	test_mapFiles  = [os.path.join(map_dir, f) for f in os.listdir(map_dir) if 'test' in f]
	class_map_file = os.path.join(data_dir, "classInd.txt")
