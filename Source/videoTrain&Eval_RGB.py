...
def train_model(network_path, train_reader, output_dir, log_file):
    ...
    for index in range(len(z.outputs)):
        print("Index {} for output: {} | {}.".format(index, base_model.outputs[index].name, base_model.outputs[index].shape))
    ...
    sample_count = 0
    for epoch in xrange(max_epochs):      # loop over epochs
        train_reader.reset()
...
if __name__ == '__main__':
    ...
    # test_model = os.path.join("F:\TCC\Models\ResNet34_videoRGB2")
